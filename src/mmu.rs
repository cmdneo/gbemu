...
impl Mmu {
    pub(crate) fn new(cartd: Cartidge) -> Self {
        ...
        // TODO Implement CGB mode settings and DMA.
        Self {
            cart: cartd,
    ...
    fn read_reg(&self, addr: usize) -> u8 {
        match addr {
            ...
            // IO_HDMA5 => {}
            IO_DMA => self.dma,
            IO_KEY1 if self.cart.is_cgb => self.key1.read(),
    ...
    fn write_reg(&mut self, addr: usize, v: u8) {
        ...
        match addr {
            IO_JOYPAD => {
                set!(self.joypad, v, mask_u8(4));
                self.update_joypad(self.dpad, self.buttons);
            }

            IO_SB => self.serial.sb = v,
            IO_SC => set!(self.serial.sc, v, mask_u8(5) << 2),
            IO_DIV => self.timer.reset_div(), // writing to DIV resets it
            IO_TIMA => self.timer.tima = v,
            IO_TMA => self.timer.tma = v,
            IO_TAC => self.timer.tac.write(v),
            IO_IF => set!(self.iflag, v, !mask_u8(5)),
            IO_IE => set!(self.ienable, v, !mask_u8(5)),
            ...
            IO_NR14 => set!(self.apu.ch1.nx4, v, mask_u8(3) << 3),

            IO_NR21 => self.apu.ch2.nx1.write(v),
            IO_NR22 => self.apu.ch2.nx2.write(v),
            IO_NR23 => self.apu.ch2.nx3.period_low = v,
            IO_NR24 => set!(self.apu.ch2.nx4, v, mask_u8(3) << 3),

            IO_NR30 => set!(self.apu.ch3.n30, v, mask_u8(7)),
            IO_NR31 => self.apu.ch3.n31.length_period = v,
            IO_NR32 => set!(self.apu.ch3.n32, v, 1 << 7 | mask_u8(5)),
            IO_NR33 => self.apu.ch3.n33.period_low = v,
            IO_NR34 => set!(self.apu.ch3.n34, v, mask_u8(3) << 3),

            IO_NR41 => set!(self.apu.ch4.n41, v, mask_u8(2) << 6),
            IO_NR42 => self.apu.ch4.n42.write(v),
            IO_NR43 => self.apu.ch4.write_n43(v),
            IO_NR44 => set!(self.apu.ch4.n44, v, mask_u8(6)),

            IO_NR50 => self.apu.nr50.write(v),
            IO_NR51 => self.apu.nr51.write(v),
            IO_NR52 => set!(self.apu.nr52, v, mask_u8(7)),

            IO_LCDC => self.ppu.fetcher.lcdc.write(v),
            IO_STAT => set!(self.ppu.stat, v, mask_u8(3)),
            ...
            IO_BGPD => {
                ...
                if self.bgpi.auto_inc == 1 {
                    self.bgpi.addr = (self.bgpi.addr + 1) & mask_u8(6);
                }
            }
            IO_OBPD => {
                self.ppu.obj_palette[self.obpi.addr as usize] = v;
                if self.obpi.auto_inc == 1 {
                    self.obpi.addr = (self.obpi.addr + 1) & mask_u8(6);
                }
            }

            IO_OPRI => self.opri = v & 1,
            IO_SVBK if self.cart.is_cgb => {
                self.wram_idx = if v == 0 { 1 } else { (v & mask_u8(3)) as usize }
            }
            IO_VBK if self.cart.is_cgb => self.vram_idx = (v as usize) & 1,
            ...
            IO_KEY1 if self.cart.is_cgb => set!(self.key1, v, !mask_u8(1)),
    ...
    pub(crate) fn update_joypad(&mut self, dpad: DPad, btns: ActionButtons) {
        let mut new = mask_u8(4); // In Joypad 0-bit means pressed.
        ...
        if (self.joypad.state & !new) & mask_u8(4) != 0 {
...
fn is_cart_addr(addr: usize) -> bool {
    addr <= *ADDR_ROM1.end() || ADDR_EXT_RAM.contains(&addr)
}
